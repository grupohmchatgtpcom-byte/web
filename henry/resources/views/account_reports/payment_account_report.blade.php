    ...
    <section class="content no-print">
        ...
        @if ($bcvRate > 0)
            <div class="alert alert-info">
                ...
                @endif
                <input type="hidden" id="payment_account_bcv_rate" value="{{ $bcvRate }}">
            </div>
        @endif
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="well well-sm">
                    ...
                    <span id="payment_account_total_usd">-</span>
                    <span id="payment_account_total_bs" class="text-muted"></span>
                </div>
        ...
        $(document).ready(function() {
            ...
            payment_account_report = $('#payment_account_report').DataTable({
                ...
                "fnDrawCallback": function(oSettings) {
                    ...
                    var totalAmount = 0;
                    $('#payment_account_report').find('span.payment-amount').each(function() {
                        var value = parseFloat($(this).data('orig-value') || 0);
                        totalAmount += value;
                    });

                    $('#payment_account_total_usd').html(__currency_trans_from_en(totalAmount, true));

                    var bcvRate = parseFloat($('#payment_account_bcv_rate').val() || 0);
                    if (bcvRate > 0) {
                        var totalBs = totalAmount * bcvRate;
                        $('#payment_account_total_bs').html(' | Bs ' + __number_f(totalBs, false, false, __currency_precision));
                    } else {
                        $('#payment_account_total_bs').html('');
                    }
                }
            });
