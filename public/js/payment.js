$(document).ready(function() {
    // GHM: Rellenar tasa BCV activa en el modal de pago manual
    function ghm_fill_bcv_rate_in_payment_modal(container) {
        var rate = 0;
        if ($('#dashboard_bcv_rate').length) {
            rate = parseFloat($('#dashboard_bcv_rate').val()) || 0;
        }
        if (rate <= 0) {
            // Intentar leer del endpoint si no tenemos el valor
            $.ajax({
                url: '/bcv-rate/current',
                dataType: 'json',
                success: function(data) {
                    if (data && data.rate) {
                        rate = parseFloat(data.rate) || 1;
                        container.find('#tp_exchange_rate_used').val(rate);
                    }
                }
            });
        } else {
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.status == 'due') {
                    ...
                    ghm_fill_bcv_rate_in_payment_modal(container);
