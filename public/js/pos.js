...
$(document).ready(function() {
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        if ($('#pos_credit_panel').length) {
            updateCreditPanel(data.id);
        }
    });
    ...
    $('button#pos-suspend').click(function() {
        var tag = ($('#suspend_label_input').val() || '').trim();
        var notesEl = $('textarea[name="additional_notes"]');
        var notes = (notesEl.val() || '').trim();
        // Remueve prefijo [ETIQUETA] previo para evitar duplicados al re-suspender.
        notes = notes.replace(/^\[[^\]]+\]\s*/g, '');
        if (tag.length > 0) {
            tag = tag.toUpperCase();
            $('#suspend_label').val(tag);
            notes = '[' + tag + '] ' + notes;
        } else {
            $('#suspend_label').val('');
        }
        notesEl.val(notes.trim());
...
function set_default_customer() {
    ...
    // Panel de crédito para cliente por defecto / preseleccionado (edición)
    if ($('#pos_credit_panel').length && default_customer_id) {
        updateCreditPanel(default_customer_id);
    }
...
function updateCreditPanel(contactId) {
    var panel = $('#pos_credit_panel');
    if (!panel.length || !contactId) {
        if (panel.length) panel.addClass('hide');
        return;
    }
    // Reset visual antes del fetch
    panel.addClass('hide');
    $('#pos_credit_bar').css('width', '0%').removeClass('progress-bar-danger progress-bar-warning progress-bar-success').addClass('progress-bar-success');
    $.ajax({
        method: 'GET',
        url: '/contacts/credit-status/' + contactId,
        dataType: 'json',
        success: function(data) {
            if (!data || data.credit_limit === null || data.credit_limit <= 0) {
                panel.addClass('hide');
                return;
            }
            var limit    = data.credit_limit  || 0;
            var used     = data.total_due     || 0;
            var avail    = data.available     || 0;
            var pct      = data.used_pct      || 0;
            var fmt = function(v) { return __currency_trans_from_en(v, true); };
            $('#pos_credit_limit_val').text(fmt(limit));
            $('#pos_credit_used_val').text(fmt(used));
            $('#pos_credit_avail_val').text(fmt(avail));
            $('#pos_credit_pct_text').text('Cr\u00e9dito utilizado: ' + pct.toFixed(1) + '%');
            var bar = $('#pos_credit_bar');
            bar.css('width', Math.min(pct, 100) + '%')
               .removeClass('progress-bar-danger progress-bar-warning progress-bar-success');
            if (pct >= 90) {
                bar.addClass('progress-bar-danger');
                $('#pos_credit_avail_badge').removeClass('label-success label-warning').addClass('label-danger');
            } else if (pct >= 60) {
                bar.addClass('progress-bar-warning');
                $('#pos_credit_avail_badge').removeClass('label-success label-danger').addClass('label-warning');
            } else {
                bar.addClass('progress-bar-success');
                $('#pos_credit_avail_badge').removeClass('label-danger label-warning').addClass('label-success');
            }
