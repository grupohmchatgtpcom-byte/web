$(document).ready(function() {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        fnDrawCallback: function(oSettings) {
            ...
            var bcvRate = parseFloat($('#contacts_report_bcv_rate').val() || 0);
            if (bcvRate > 0) {
                var appendBs = function(selector, usdAmount) {
                    var bsAmount = parseFloat(usdAmount || 0) * bcvRate;
                    $(selector).append('<br><small class="text-muted">Bs ' + __number_f(bsAmount, false, false, __currency_precision) + '</small>');
                };

                appendBs('#footer_total_purchase', total_purchase);
                appendBs('#footer_total_purchase_return', total_purchase_return);
                appendBs('#footer_total_sell', total_sell);
                appendBs('#footer_total_sell_return', total_sell_return);
                appendBs('#footer_total_opening_bal_due', total_opening_bal_due);
                appendBs('#footer_total_due', total_due);
            }
        },
    });
...
function updatePurchaseSell() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            } else if ($('#purchase_sell_bcv_rate').length) {
                bcvRate = parseFloat($('#purchase_sell_bcv_rate').val() || 0);
            }
            ...
            $('.total_purchase').html(renderWithBs(data.purchase.total_purchase_exc_tax));
            $('.purchase_inc_tax').html(renderWithBs(data.purchase.total_purchase_inc_tax));
            $('.purchase_due').html(renderWithBs(data.purchase.purchase_due));

            $('.total_sell').html(renderWithBs(data.sell.total_sell_exc_tax));
            $('.sell_inc_tax').html(renderWithBs(data.sell.total_sell_inc_tax));
            $('.sell_due').html(renderWithBs(data.sell.invoice_due));
            $('.purchase_return_inc_tax').html(renderWithBs(data.total_purchase_return));
            $('.total_sell_return').html(renderWithBs(data.total_sell_return));

            $('.sell_minus_purchase').html(renderWithBs(data.difference.total));
            __highlight(data.difference.total, $('.sell_minus_purchase'));

            $('.difference_due').html(renderWithBs(data.difference.due));
