@extends('layouts.app')
@section('title', 'Libro de Compras — SENIAT')
...
<section class="content no-print">
    ...
    @component('components.filters', ['title' => 'Período'])
        <div class="col-md-3">
            <div class="form-group">
                {!! Form::label('lc_start', 'Desde:') !!}
                {!! Form::text('lc_start', $start_date, ['class'=>'form-control','id'=>'lc_start','readonly']) !!}
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
                {!! Form::label('lc_end', 'Hasta:') !!}
                {!! Form::text('lc_end', $end_date, ['class'=>'form-control','id'=>'lc_end','readonly']) !!}
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group">
                <label>&nbsp;</label><br>
                <button class="btn btn-primary btn-sm" id="lc_apply"><i class="fa fa-search"></i> Consultar</button>
                <a href="{{ route('reports.libro_compras') }}?start_date={{ $start_date }}&end_date={{ $end_date }}&export=csv"
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            <div class="info-box bg-aqua">
                <div class="info-box-content">
                    <span class="info-box-text">Total Compras Bs</span>
    ...
    @component('components.widget', ['class' => 'box-primary'])
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-condensed" id="lc_table" style="font-size:12px;">
                ...
                <tbody>
                    ...
                    @empty
                    <tr><td colspan="11" class="text-center">No hay facturas de compra en este período.</td></tr>
                    @endforelse
                </tbody>
...
$(document).ready(function() {
    $('#lc_start, #lc_end').datepicker({ dateFormat: 'yy-mm-dd', changeMonth: true, changeYear: true });
    $('#lc_apply').on('click', function() {
        window.location.href = '{{ route("reports.libro_compras") }}?start_date=' + $('#lc_start').val() + '&end_date=' + $('#lc_end').val();
    });
    if ($('#lc_table tbody tr').length) {
        $('#lc_table').DataTable({
            paging: false,
            order: [[2, 'asc']],
