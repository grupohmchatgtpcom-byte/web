        ...
        <div class="ticket">
			...
			@if(empty($receipt_details->hide_price))
				...
				@if(!empty($receipt_details->bcv_rate_value) && $receipt_details->bcv_rate_value > 0)
					<div class="flex-box">
						<p class="width-50 text-right"><small>Tasa BCV @if(!empty($receipt_details->bcv_rate_date))({{$receipt_details->bcv_rate_date}})@endif</small></p>
					...
					@if(!empty($receipt_details->total_bs_display))
						<div class="flex-box">
							<p class="width-50 text-right sub-headings">Total estimado (Bs)</p>
							<p class="width-50 text-right sub-headings">Bs {{$receipt_details->total_bs_display}}</p>
						</div>
					@endif
				@endif
