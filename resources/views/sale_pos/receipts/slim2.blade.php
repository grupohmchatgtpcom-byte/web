...
<html lang="en">
    ...
    <body>
        <div class="ticket">
			...
			@if(!empty($receipt_details->bcv_rate_value) && $receipt_details->bcv_rate_value > 0)
				<div class="flex-box">
					<p class="width-50 text-left"><small>Tasa BCV @if(!empty($receipt_details->bcv_rate_date))({{$receipt_details->bcv_rate_date}})@endif</small></p>
					<p class="width-50 text-right"><small>{{ number_format((float) $receipt_details->bcv_rate_value, 2, ',', '.') }} Bs/USD</small></p>
				</div>
				@if(!empty($receipt_details->total_bs_display))
					<div class="flex-box">
						<p class="width-50 text-left"><strong>Total estimado (Bs)</strong></p>
						<p class="width-50 text-right"><strong>Bs {{$receipt_details->total_bs_display}}</strong></p>
					</div>
				@endif
			@endif
			...
			@if(!empty($receipt_details->payments))
				@foreach($receipt_details->payments as $payment)
					...
					@if(!empty($payment['amount_in_bs']) && $payment['amount_in_bs'] > 0)
						<div class="flex-box" style="font-size:9px; color:#555;">
							<p class="width-50 text-left" style="padding-left:8px;">
								@if(!empty($payment['currency_code']) && $payment['currency_code'] === 'VES')
									Bs (recibido)
								@else
									Bs (equiv. tasa {{ number_format($payment['exchange_rate_used'] ?? 0, 2) }})
								@endif
							</p>
							<p class="width-50 text-right">Bs {{ number_format($payment['amount_in_bs'], 2) }}</p>
						</div>
					@endif
				@endforeach
			@endif
