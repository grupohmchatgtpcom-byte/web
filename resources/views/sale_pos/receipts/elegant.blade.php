	...
	<div class="col-md-6 invoice-col width-50">
		<table class="table-no-side-cell-border table-no-top-cell-border width-100 table-slim">
			<tbody>
				...
				<tr>
					<td>Tasa BCV @if(!empty($receipt_details->bcv_rate_date)) ({{$receipt_details->bcv_rate_date}}) @endif</td>
					<td class="text-right">{{ number_format((float) $receipt_details->bcv_rate_value, 4, '.', ',') }} Bs/USD</td>
				</tr>
				@if(!empty($receipt_details->total_bs_display))
				<tr>
					<td>Total estimado (Bs)</td>
					<td class="text-right">Bs {{$receipt_details->total_bs_display}}</td>
				</tr>
				@endif
				@endif
