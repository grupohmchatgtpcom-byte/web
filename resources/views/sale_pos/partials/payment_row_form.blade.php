<div class="row">
	...
	@php
		...
		// Detectar moneda por defecto según el método guardado
		$default_currency_code = !empty($payment_line['currency_code']) ? $payment_line['currency_code'] : 'USD';
	@endphp

	{{-- Campos ocultos de dualidad de monedas --}}
	{!! Form::hidden("payment[$row_index][currency_code]", $default_currency_code, [
		'class' => 'payment-currency-code',
		'id' => "currency_code_$row_index",
	]) !!}
	{!! Form::hidden("payment[$row_index][exchange_rate_used]", !empty($payment_line['exchange_rate_used']) ? $payment_line['exchange_rate_used'] : 0, [
		'class' => 'payment-exchange-rate',
		'id' => "exchange_rate_used_$row_index",
	]) !!}
	...
	{{-- Equivalente en la otra moneda (solo lectura, calculado por JS) --}}
	<div class="col-md-12 payment-equiv-row" id="equiv_row_{{ $row_index }}" style="margin-bottom:6px;">
		<div class="alert alert-info alert-sm tw-py-1 tw-px-2 tw-text-sm tw-mb-0">
			<span class="payment-equiv-label">Equivalente:</span>
			<strong class="payment-equiv-value" id="equiv_value_{{ $row_index }}">—</strong>
			<span class="payment-equiv-currency" id="equiv_currency_{{ $row_index }}"></span>
		</div>
	</div>
	@if(!empty($show_date))
	<div class="{{$col_class}}">
	...
	{{-- Selector de moneda para este pago --}}
	<div class="{{ $col_class }}" id="currency_selector_div_{{ $row_index }}">
		<div class="form-group">
			{!! Form::label("currency_display_$row_index", 'Moneda del pago:') !!}
			<div class="input-group">
				<span class="input-group-addon"><i class="fas fa-dollar-sign"></i></span>
				<select class="form-control payment-currency-selector"
					id="currency_display_{{ $row_index }}"
					data-row-index="{{ $row_index }}"
					style="width:100%;">
					<option value="USD" {{ $default_currency_code === 'USD' ? 'selected' : '' }}>USD — Dólares</option>
					<option value="VES" {{ $default_currency_code === 'VES' ? 'selected' : '' }}>VES — Bolívares</option>
				</select>
			</div>
		</div>
	</div>
