@php
    $is_mobile = isMobile();
    $bcvRateValue = !empty($bcv_rate_info['usd_to_ves_rate']) ? (float) $bcv_rate_info['usd_to_ves_rate'] : 0;
    if ($bcvRateValue <= 0 && !empty($header_bcv_rate_info['usd_to_ves_rate'])) {
        $bcvRateValue = (float) $header_bcv_rate_info['usd_to_ves_rate'];
    }
@endphp
<input type="hidden" id="pos_bcv_rate" value="{{ $bcvRateValue }}">
<div class="row">
    <div
        ...
        <div
            ...
            <div class="tw-flex tw-items-center tw-gap-4 tw-flex-row tw-overflow-x-auto">
                ...
                @if (!$is_mobile)
                    ...
                    <div class="pos-total md:tw-flex md:tw-items-center md:tw-gap-3 tw-hidden">
                        <div
                            class="tw-text-black tw-font-bold tw-text-base md:tw-text-2xl tw-flex tw-flex-col tw-items-start tw-leading-tight">
                            <div>Total Payable:</div>
                            @if ($bcvRateValue > 0)
                                <div class="tw-text-xs md:tw-text-sm tw-text-gray-700 tw-mt-1">Total Bs:</div>
                            @endif
                        </div>
                        <div class="tw-flex tw-flex-col tw-items-start tw-leading-tight">
                            ...
                            @if ($bcvRateValue > 0)
                                <span class="pos_total_bs_display tw-text-blue-800 tw-font-bold tw-text-sm md:tw-text-lg tw-mt-1">0.00</span>
                            @endif
                        </div>
                    </div>
                @endif
            ...
            <div class="tw-w-full md:tw-w-fit tw-flex tw-flex-col tw-items-end tw-gap-3 tw-hidden md:tw-block">
                @if ($bcvRateValue > 0)
                    <div class="tw-text-right tw-text-xs tw-text-gray-600 tw-mb-2 js-bcv-container">
                        <div>
                            <strong>Tasa BCV:</strong>
                            1 USD = {{ number_format($bcvRateValue, 4, '.', ',') }} Bs
                            @if (!empty($bcv_rate_info['rate_date']))
                                <span>| {{ $bcv_rate_info['rate_date'] }}</span>
                            @endif
                        </div>
                    </div>
                @endif
