        ...
        <div class="tw-px-5 tw-pt-3">
            ...
            @if (auth()->user()->can('dashboard.data'))
                @if ($is_admin)
                    ...
                    @php
                        $bcvRate = !empty($bcv_rate_info['usd_to_ves_rate']) ? (float) $bcv_rate_info['usd_to_ves_rate'] : 0;
                    @endphp
                    @if ($bcvRate > 0)
                        <div class="alert alert-info tw-mt-4">
                            <strong>Tasa BCV de referencia:</strong>
                            1 USD = {{ number_format($bcvRate, 4, '.', ',') }} Bs
                            @if (!empty($bcv_rate_info['rate_date']))
                                | Fecha: {{ $bcv_rate_info['rate_date'] }}
                            @endif
                            @if (!empty($bcv_rate_info['is_fallback']))
                                | <span class="text-warning">Fallback</span>
                            @endif
                            <input type="hidden" id="dashboard_bcv_rate" value="{{ $bcvRate }}">
                        </div>
                    @endif
    ...
    @if (auth()->user()->can('dashboard.data'))
        <div class="tw-px-5 tw-py-6">
            <div class="tw-grid tw-grid-cols-1 tw-gap-4 sm:tw-gap-5 lg:tw-grid-cols-2">
                ...
                {{-- ===== DASHBOARD GERENCIAL ===== --}}
                @if ($is_admin)
                    {{-- Ventas de hoy (USD y Bs) --}}
                    <div class="tw-col-span-1 lg:tw-col-span-2">
                        <div class="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4">
                            <div class="tw-bg-white tw-rounded-lg tw-shadow tw-p-5">
                                <div class="tw-flex tw-items-center tw-space-x-4">
                                    <div class="tw-flex-shrink-0 tw-bg-green-500 tw-rounded-full tw-p-3">
                                        <i class="fa fa-dollar tw-text-white tw-text-xl"></i>
                                    </div>
                                    <div>
                                        <p class="tw-text-sm tw-text-gray-500 tw-mb-0">Ventas Hoy (USD)</p>
                                        <p id="gerencial_today_usd" class="tw-text-2xl tw-font-bold tw-text-gray-800 tw-mb-0">--</p>
                                    </div>
                                </div>
                            </div>
                            <div class="tw-bg-white tw-rounded-lg tw-shadow tw-p-5">
                                <div class="tw-flex tw-items-center tw-space-x-4">
                                    <div class="tw-flex-shrink-0 tw-bg-blue-500 tw-rounded-full tw-p-3">
                                        <i class="fa fa-money tw-text-white tw-text-xl"></i>
                                    </div>
                                    <div>
                                        <p class="tw-text-sm tw-text-gray-500 tw-mb-0">Ventas Hoy (Bs)</p>
                                        <p id="gerencial_today_bs" class="tw-text-2xl tw-font-bold tw-text-gray-800 tw-mb-0">--</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="tw-col-span-1">
                        <div class="tw-bg-white tw-rounded-lg tw-shadow tw-h-full">
                            <div class="tw-px-5 tw-py-4 tw-border-b tw-border-gray-200">
                                <h3 class="tw-text-base tw-font-semibold tw-text-gray-800 tw-mb-0">
                                    <i class="fa fa-exclamation-circle tw-text-red-500 tw-mr-1"></i> Top 5 CxC Vencidas
                                </h3>
                            </div>
                            <div class="tw-p-4 tw-overflow-x-auto">
                                <table class="tw-w-full tw-text-sm">
                                    <thead>
                                        <tr class="tw-text-xs tw-uppercase tw-text-gray-400 tw-border-b tw-border-gray-200">
                                            <th class="tw-text-left tw-pb-2 tw-pr-2">Cliente</th>
                                            <th class="tw-text-right tw-pb-2 tw-pr-2">Saldo USD</th>
                                            <th class="tw-text-right tw-pb-2">Días</th>
                                        </tr>
                                    </thead>
                                    <tbody id="gerencial_cxc_body">
                                        <tr><td colspan="3" class="tw-text-center tw-py-4 tw-text-gray-300">Cargando...</td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    ...
                    <div class="tw-col-span-1">
                        <div class="tw-bg-white tw-rounded-lg tw-shadow tw-h-full">
                            <div class="tw-px-5 tw-py-4 tw-border-b tw-border-gray-200">
                                <h3 class="tw-text-base tw-font-semibold tw-text-gray-800 tw-mb-0">
                                    <i class="fa fa-star tw-text-yellow-400 tw-mr-1"></i> Top 10 Productos del Mes
                                </h3>
                            </div>
                            <div class="tw-p-4 tw-overflow-x-auto">
                                <table class="tw-w-full tw-text-sm">
                                    <thead>
                                        <tr class="tw-text-xs tw-uppercase tw-text-gray-400 tw-border-b tw-border-gray-200">
                                            <th class="tw-text-left tw-pb-2 tw-pr-2">Producto</th>
                                            <th class="tw-text-right tw-pb-2 tw-pr-2">Cant.</th>
                                            <th class="tw-text-right tw-pb-2">Total USD</th>
                                        </tr>
                                    </thead>
                                    <tbody id="gerencial_products_body">
                                        <tr><td colspan="3" class="tw-text-center tw-py-4 tw-text-gray-300">Cargando...</td></tr>
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div class="tw-col-span-1 lg:tw-col-span-2">
                        <div class="tw-bg-white tw-rounded-lg tw-shadow">
                            <div class="tw-px-5 tw-py-4 tw-border-b tw-border-gray-200">
                                <h3 class="tw-text-base tw-font-semibold tw-text-gray-800 tw-mb-0">
                                    <i class="fa fa-bar-chart tw-text-indigo-500 tw-mr-1"></i> Comparativo Mensual – Últimos 6 Meses (USD)
                                </h3>
                            </div>
                            <div class="tw-p-4">
                                <canvas id="gerencial_monthly_chart" height="80"></canvas>
                            </div>
                        </div>
                    </div>
                @endif
                {{-- ===== FIN DASHBOARD GERENCIAL ===== --}}
            </div>
        </div>
    @endif
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // Ventas de hoy
                    $('#gerencial_today_usd').text('$ ' + fmt(data.today_usd));
                    $('#gerencial_today_bs').text('Bs. ' + fmt(data.today_bs));

                    // Top 5 CxC Vencidas
                    var cxcHtml = '';
                    if (data.top_overdue && data.top_overdue.length > 0) {
                        $.each(data.top_overdue, function(i, row) {
                            cxcHtml += '<tr class="tw-border-b tw-border-gray-100">' +
                                '<td class="tw-py-2 tw-pr-2 tw-text-gray-700">' +
                                    '<a href="{{ url("/reports/accounts-receivable") }}/' + row.contact_id + '" class="tw-text-blue-600 hover:tw-underline">' + row.contact_name + '</a>' +
                                '</td>' +
                                '<td class="tw-text-right tw-py-2 tw-pr-2 tw-text-red-600 tw-font-medium">$ ' + fmt(row.balance_usd) + '</td>' +
                                '<td class="tw-text-right tw-py-2 tw-text-red-400">' + row.days_overdue + 'd</td>' +
                                '</tr>';
                        });
                    } else {
                        cxcHtml = '<tr><td colspan="3" class="tw-text-center tw-py-4 tw-text-gray-400">Sin CxC vencidas</td></tr>';
                    }
                    $('#gerencial_cxc_body').html(cxcHtml);

                    // Top 10 Productos del Mes
                    var prodHtml = '';
                    if (data.top_products && data.top_products.length > 0) {
                        $.each(data.top_products, function(i, row) {
                            prodHtml += '<tr class="tw-border-b tw-border-gray-100">' +
                                '<td class="tw-py-2 tw-pr-2 tw-text-gray-700">' + row.product_name + '</td>' +
                                '<td class="tw-text-right tw-py-2 tw-pr-2 tw-text-gray-600">' + fmt(row.qty_sold, 0) + '</td>' +
                                '<td class="tw-text-right tw-py-2 tw-font-medium">$ ' + fmt(row.total_usd) + '</td>' +
                                '</tr>';
                        });
                    } else {
                        prodHtml = '<tr><td colspan="3" class="tw-text-center tw-py-4 tw-text-gray-400">Sin ventas este mes</td></tr>';
                    }
                    $('#gerencial_products_body').html(prodHtml);

                    // Gráfico comparativo mensual
                    var ctx = document.getElementById('gerencial_monthly_chart').getContext('2d');
                    new Chart(ctx, {
                        type: 'bar',
                        data: {
                                ...
                                tooltip: {
                                    callbacks: {
                                        label: function(context) {
                                            return '$ ' + parseFloat(context.raw).toLocaleString('es-VE', { minimumFractionDigits: 2 });
                                        }
                                    }
                                }
                            },
