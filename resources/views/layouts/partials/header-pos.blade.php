...
<div class="col-md-12 no-print pos-header">
    ...
    <div
        ...
        <div class="tw-w-full md:tw-w-2/3 !tw-p-0 tw-flex tw-items-center tw-justify-between tw-gap-4 tw-flex-col md:tw-flex-row tw-hidden md:tw-flex"
            ...
            <div
                class="tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-[#FFF7E6] tw-border tw-border-[#F5D58C] tw-rounded-md tw-px-3 tw-py-1 tw-text-xs tw-text-[#7A5A00]">
                <span class="tw-font-bold">Tasa del dia:</span>
                @if (!empty($header_bcv_rate_info['usd_to_ves_rate']))
                    <span class="tw-font-semibold">
                        {{ number_format((float) $header_bcv_rate_info['usd_to_ves_rate'], 2, '.', ',') }} Bs/USD
                    </span>
                    @if (!empty($header_bcv_rate_info['rate_date']))
                        <span class="tw-font-normal">({{ $header_bcv_rate_info['rate_date'] }})</span>
                    @endif
                @else
                    <span class="tw-font-semibold">Sin tasa disponible</span>
                @endif
            </div>
