...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Forzar español en toda la aplicación.
        \App::setLocale('es');

        // Observadores de auditoría operativa
        \App\Transaction::observe(TransactionObserver::class);
        \App\TransactionPayment::observe(TransactionPaymentObserver::class);
        ...
        // Compartir tasa BCV activa con todas las vistas (Fase 1 - dualidad de monedas)
        if (isAppInstalled()) {
            try {
                View::share('active_bcv_rate', getActiveBcvRate());
            } catch (\Exception $e) {
                View::share('active_bcv_rate', 1.0);
            }
        } else {
            View::share('active_bcv_rate', 1.0);
        }
        ...
        View::composer(
            ['layouts.partials.header', 'layouts.partials.header-pos'],
            function ($view) {
                $header_bcv_rate_info = null;
                    ...
                    try {
                        ...
                        foreach ($candidateTables as $table) {
                            ...
                            foreach ($candidateColumns as $rateColumn) {
                                ...
                                $header_bcv_rate_info = [
                                    'usd_to_ves_rate' => $normalizedRate,
                                    'rate_date' => ! empty($rateRow->rate_date) ? $rateRow->rate_date : null,
                                    'source' => ! empty($rateRow->source) ? $rateRow->source : null,
                                ];
                ...
                $view->with('header_bcv_rate_info', $header_bcv_rate_info);
            }
        );
