...
class AccountReportsController extends Controller
{
    ...
    public function paymentAccountReport()
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($query)
                    ...
                    ->editColumn('amount', function ($row) {
                        return '<span class="payment-amount" data-orig-value="'.$row->amount.'">'.$this->transactionUtil->num_f($row->amount, true).'</span>';
                    })
                    ...
                    ->rawColumns(['action', 'transaction_number', 'details', 'amount'])
                    ->make(true);
        }
        ...
        return view('account_reports.payment_account_report')
                ->with(compact('accounts', 'bcv_rate_info'));
    }
