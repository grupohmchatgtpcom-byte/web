...
class ReportController extends Controller
{
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        return view('report.purchase_sell')
                    ->with(compact('business_locations', 'bcv_rate_info'));
    }
    ...
    protected function getCurrentBcvRate(): ?array
    {
        ...
        $rateRow = DB::table('ghm_bcv_rates')
        ...
        return [
            ...
            'usd_to_ves_rate' => (float) $rateRow->usd_to_ves_rate,
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        ->with(compact('customer_group', 'types', 'business_locations', 'contact_dropdown', 'bcv_rate_info'));
    }
    ...
    /**
     * Reporte de ventas diarias con desglose por método de pago y gráfico por hora.
     */
    public function dailySales(Request $request)
    {
        ...
        if ($location_id) {
            $base->where('transactions.location_id', $location_id);
        }

        // Totales del día
        $totals = (clone $base)
            ->selectRaw('COUNT(*) as total_transactions, COALESCE(SUM(final_total),0) as total_usd')
            ->first();

        // Totales de ayer (comparación)
        $yesterday     = \Carbon\Carbon::parse($date)->subDay()->toDateString();
        $yesterdayBase = DB::table('transactions')
            ->where('business_id', $business_id)
            ->whereDate('transaction_date', $yesterday)
            ->where('type', 'sell')
            ->whereIn('status', ['final']);
        if ($location_id) {
            $yesterdayBase->where('location_id', $location_id);
        }
        $yesterday_usd = $yesterdayBase->sum('final_total');

        $summary = [
            'total_usd'          => (float)($totals->total_usd ?? 0),
            'total_bs'           => (float)($totals->total_usd ?? 0) * $bcv_rate,
            'total_transactions' => (int)($totals->total_transactions ?? 0),
            'yesterday_usd'      => (float)$yesterday_usd,
        ];

        // Por método de pago
        $by_payment_method = DB::table('transaction_payments as tp')
            ...
            ->selectRaw('COALESCE(tp.method,"efectivo") as method_name,
                         COUNT(*) as count,
                         COALESCE(SUM(CASE WHEN tp.currency_code="VES" THEN tp.amount/NULLIF(tp.exchange_rate_used,0) ELSE tp.amount END),0) as total_usd,
                         COALESCE(SUM(tp.amount_in_bs),0) as total_bs')
            ->groupBy('tp.method')
            ->get();

        // Por hora
        $hourly_data = (clone $base)
            ->selectRaw('HOUR(transaction_date) as hour, COALESCE(SUM(final_total),0) as total_usd')
            ->groupByRaw('HOUR(transaction_date)')
            ->orderByRaw('HOUR(transaction_date)')
            ->get();
        ...
        $transactions = DB::table('transactions as t')
            ...
            ->whereIn('t.status', ['final'])
            ->when($location_id, fn($q) => $q->where('t.location_id', $location_id))
            ->selectRaw('t.id, t.invoice_no, t.transaction_date, t.final_total, t.payment_status,
                         c.name as contact_name,
                         t.final_total * ? as total_bs', [$bcv_rate])
            ->orderBy('t.transaction_date')
            ->get();

        $locations = BusinessLocation::forDropdown($business_id, true);

        return view('report.daily_sales', compact(
            'date', 'summary', 'by_payment_method', 'hourly_data',
            'transactions', 'locations', 'location_id', 'bcv_rate'
        ));
    }

    /**
     * Reporte de ventas mensuales agrupado por día con chart de tendencia.
     */
    public function monthlySales(Request $request)
    {
        ...
        // Ventas por día
        $daily_data = DB::table('transactions')
            ->where('business_id', $business_id)
            ...
            ->selectRaw('DATE(transaction_date) as sale_date,
                         COUNT(*) as transactions,
                         COALESCE(SUM(final_total),0) as total_usd,
                         COALESCE(SUM(final_total),0) * ? as total_bs', [$bcv_rate])
        ...
        $prev_month_usd = DB::table('transactions')
            ->where('business_id', $business_id)
            ->whereBetween(DB::raw('DATE(transaction_date)'), [$prevStart->toDateString(), $prevEnd->toDateString()])
            ->where('type', 'sell')
            ->whereIn('status', ['final'])
            ->when($location_id, fn($q) => $q->where('location_id', $location_id))
            ->sum('final_total');
        ...
        $summary = [
            'total_usd'          => $daily_data->sum('total_usd'),
            'total_bs'           => $daily_data->sum('total_bs'),
            'total_transactions' => $daily_data->sum('transactions'),
            'days_with_sales'    => $daily_data->count(),
            'prev_month_usd'     => (float)$prev_month_usd,
        ];

        // Por método de pago en el mes
        $by_payment_method = DB::table('transaction_payments as tp')
            ->join('transactions as t', 't.id', '=', 'tp.transaction_id')
            ->where('t.business_id', $business_id)
            ...
            ->selectRaw('COALESCE(tp.method,"efectivo") as method_name,
                         COALESCE(SUM(CASE WHEN tp.currency_code="VES" THEN tp.amount/NULLIF(tp.exchange_rate_used,0) ELSE tp.amount END),0) as total_usd')
            ->groupBy('tp.method')
            ->orderByRaw('total_usd DESC')
            ->get();

        $locations = BusinessLocation::forDropdown($business_id, true);

        return view('report.monthly_sales', compact(
            'month', 'year', 'daily_data', 'summary', 'by_payment_method',
            'locations', 'location_id', 'bcv_rate'
        ));
    }

    /**
     * Reporte de Compras por Período, agrupado por proveedor y categoría.
     * GET /reports/purchases-period
     */
    public function purchasesByPeriod(Request $request)
    {
        if (! auth()->user()->can('purchase.view') && ! auth()->user()->can('view_own_purchase')) {
            abort(403, 'No autorizado');
        }
        ...
        // Exportación CSV
        if ($request->get('export') === 'csv' && ! $request->ajax()) {
            $rows = DB::table('transactions as t')
                ...
                ->when($supplier_id, fn($q) => $q->where('t.contact_id', $supplier_id))
                ->selectRaw('
                    c.name as proveedor,
                    c.contact_id as rif,
                    COUNT(t.id) as total_ordenes,
                    COALESCE(SUM(t.total_before_tax), 0) as base_imponible,
                    COALESCE(SUM(t.tax_amount), 0) as total_iva,
                    COALESCE(SUM(t.final_total), 0) as total_usd,
                    COALESCE(SUM(t.final_total), 0) * ? as total_bs,
                    MAX(t.transaction_date) as ultima_compra
                ', [$bcv_rate])
                ->groupBy('c.id', 'c.name', 'c.contact_id')
                ->orderByRaw('total_usd DESC')
                ->get();

            $filename = 'compras_periodo_' . $start_date . '_al_' . $end_date . '.csv';
            $headers  = [
                'Content-Type'        => 'text/csv; charset=UTF-8',
                'Content-Disposition' => 'attachment; filename="' . $filename . '"',
            ];
            return response()->stream(function () use ($rows, $start_date, $end_date, $bcv_rate) {
                $out = fopen('php://output', 'w');
                fprintf($out, chr(0xEF) . chr(0xBB) . chr(0xBF));
                fputcsv($out, ['Compras por Período — ' . $start_date . ' al ' . $end_date . ' — Tasa BCV: ' . number_format($bcv_rate, 2)]);
                fputcsv($out, []);
                fputcsv($out, ['Proveedor', 'RIF', 'Órdenes', 'Base Imponible USD', 'IVA USD', 'Total USD', 'Total Bs', 'Última Compra']);
                foreach ($rows as $r) {
                    fputcsv($out, [
                        $r->proveedor,
                        $r->rif ?? '',
                        ...
                        number_format($r->total_iva, 2, '.', ''),
                        number_format($r->total_usd, 2, '.', ''),
                        number_format($r->total_bs, 2, '.', ''),
                        $r->ultima_compra ?? '',
                    ]);
                }
                fputcsv($out, []);
                fputcsv($out, ['TOTAL', '', $rows->sum('total_ordenes'),
                    number_format($rows->sum('base_imponible'), 2, '.', ''),
                    number_format($rows->sum('total_iva'), 2, '.', ''),
                    number_format($rows->sum('total_usd'), 2, '.', ''),
                    number_format($rows->sum('total_bs'), 2, '.', ''), '']);
                fclose($out);
            }, 200, $headers);
        }
        ...
        if ($request->ajax()) {
            $query = DB::table('transactions as t')
                ...
                ->selectRaw('
                    ...
                    COALESCE(SUM(t.tax_amount), 0) as total_iva,
                    COALESCE(SUM(t.total_before_tax), 0) as base_imponible,
                    MIN(t.transaction_date) as primera_compra,
                    MAX(t.transaction_date) as ultima_compra
                ', [$bcv_rate])
                ->groupBy('c.id', 'c.name', 'c.contact_id');
            ...
            return datatables()->of($query)
                ->addColumn('acciones', function ($row) {
                    return '<a href="' . url('/reports/purchases-period/' . $row->supplier_id) . '"
        ...
        // Para el resumen general (no ajax)
        $summary = DB::table('transactions as t')
            ...
            ->selectRaw('
                ...
                COALESCE(SUM(final_total), 0) as total_usd,
                COALESCE(SUM(tax_amount), 0) as total_iva,
                COALESCE(SUM(total_before_tax), 0) as base_imponible
            ')
            ->first();

        $summary->total_bs = ($summary->total_usd ?? 0) * $bcv_rate;

        $suppliers  = Contact::suppliersDropdown($business_id);
        $locations  = BusinessLocation::forDropdown($business_id, true);

        return view('report.purchases_period', compact(
            'start_date', 'end_date', 'summary', 'suppliers', 'locations',
            'supplier_id', 'bcv_rate'
        ));
    }

    /**
     * Libro de Ventas — formato SENIAT Venezuela.
    ...
    public function libroVentas(Request $request)
    {
        ...
        // Facturas de ventas finalizadas en el período
        $facturas = DB::table('transactions as t')
            ...
            ->selectRaw("
                ...
                t.invoice_no,
                t.transaction_date,
                COALESCE(c.contact_id, '') as rif_cliente,
                COALESCE(c.name, 'CONSUMIDOR FINAL') as nombre_cliente,
                COALESCE(t.total_before_tax, t.final_total) as base_imponible,
                COALESCE(t.tax_amount, 0) as monto_iva,
                COALESCE(t.final_total, 0) as total_factura,
                COALESCE(t.final_total, 0) * {$bcv_rate} as total_factura_bs,
                CASE WHEN COALESCE(t.tax_amount, 0) = 0 THEN t.final_total ELSE 0 END as monto_exento,
                CASE WHEN t.tax_rate_id IS NOT NULL THEN 16 ELSE 0 END as alicuota_iva
            ")
            ->orderBy('t.transaction_date')
            ->orderBy('t.id')
            ->get();
        ...
        // Totales del período
        $totales = [
            'base_imponible' => $facturas->sum('base_imponible'),
            'monto_iva'      => $facturas->sum('monto_iva'),
            'total_facturas' => $facturas->sum('total_factura'),
            'total_bs'       => $facturas->sum('total_factura_bs'),
            'monto_exento'   => $facturas->sum('monto_exento'),
        ...
        if ($request->get('export') === 'csv') {
            return $this->exportLibroVentasCsv($facturas, $start_date, $end_date);
        }

        return view('report.libro_ventas', compact(
            'facturas', 'totales', 'start_date', 'end_date', 'locations', 'bcv_rate'
        ));
    }

    /**
     * Exportar Libro de Ventas a CSV (con BOM para Excel).
     */
    private function exportLibroVentasCsv($facturas, string $start_date, string $end_date)
    {
        $filename = "libro_ventas_{$start_date}_{$end_date}.csv";
        $headers  = [
            'Content-Type'        => 'text/csv; charset=UTF-8',
            'Content-Disposition' => "attachment; filename=\"{$filename}\"",
        ];
        $callback = function () use ($facturas) {
            ...
            fputcsv($handle, [
                'N° Control', 'N° Factura', 'Fecha', 'RIF Cliente', 'Razón Social',
                'Base Imponible ($)', 'Alícuota IVA %', 'Monto IVA ($)', 'Exento ($)', 'Total ($)', 'Total Bs',
            ], ';');
            foreach ($facturas as $f) {
                fputcsv($handle, [
                    $f->id,
                    $f->invoice_no,
                    \Carbon\Carbon::parse($f->transaction_date)->format('d/m/Y'),
                    $f->rif_cliente,
                    $f->nombre_cliente,
                    number_format($f->base_imponible, 2, '.', ''),
                    $f->alicuota_iva,
                    number_format($f->monto_iva, 2, '.', ''),
                    number_format($f->monto_exento, 2, '.', ''),
                    number_format($f->total_factura, 2, '.', ''),
                    number_format($f->total_factura_bs, 2, '.', ''),
                ], ';');
            }
            fclose($handle);
        };
    ...
    /**
     * Libro de Compras — formato SENIAT Venezuela.
     * GET /reports/libro-compras
     */
    public function libroCompras(Request $request)
    {
        ...
        $facturas = DB::table('transactions as t')
            ...
            ->selectRaw("
                t.id,
                t.ref_no as numero_factura,
                t.transaction_date,
                COALESCE(c.contact_id, '') as rif_proveedor,
                COALESCE(c.name, '') as nombre_proveedor,
                COALESCE(t.total_before_tax, t.final_total) as base_imponible,
                COALESCE(t.tax_amount, 0) as monto_iva,
                COALESCE(t.final_total, 0) as total_factura,
                COALESCE(t.final_total, 0) * {$bcv_rate} as total_factura_bs,
        ...
        if ($request->get('export') === 'csv') {
            return $this->exportLibroComprasCsv($facturas, $start_date, $end_date);
        }

        return view('report.libro_compras', compact(
            'facturas', 'totales', 'start_date', 'end_date', 'bcv_rate'
        ));
    }
     ...
     */
    private function exportLibroComprasCsv($facturas, string $start_date, string $end_date)
    {
        $filename = "libro_compras_{$start_date}_{$end_date}.csv";
        $headers  = [
            'Content-Type'        => 'text/csv; charset=UTF-8',
            'Content-Disposition' => "attachment; filename=\"{$filename}\"",
        ];
        $callback = function () use ($facturas) {
            ...
            fputcsv($handle, [
                'N° Control', 'N° Factura Proveedor', 'Fecha', 'RIF Proveedor', 'Razón Social',
                'Base Imponible ($)', 'Alícuota IVA %', 'Monto IVA ($)', 'Exento ($)', 'Total ($)', 'Total Bs',
            ], ';');
            foreach ($facturas as $f) {
                fputcsv($handle, [
    ...
    /**
     * Cierre de Caja mejorado con desglose por moneda.
     * GET /reports/cash-register-close
     */
    public function cashRegisterClose(Request $request)
    {
        ...
        // Registro de caja del día (si existe)
        ...
        // Ventas del día
        $ventas_base = DB::table('transactions as t')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->whereIn('t.status', ['final'])
            ->whereDate('t.transaction_date', $date)
        ...
        $total_ventas_usd = (clone $ventas_base)->sum('t.final_total');
        $total_transacciones = (clone $ventas_base)->count();

        // Cobros por método de pago
        $cobros_por_metodo = DB::table('transaction_payments as tp')
            ->join('transactions as t', 't.id', '=', 'tp.transaction_id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->whereIn('t.status', ['final'])
            ...
            ->when($location_id, fn($q) => $q->where('t.location_id', $location_id))
            ->selectRaw("
                COALESCE(tp.method, 'efectivo') as metodo,
                COALESCE(tp.currency_code, 'USD') as moneda,
                COUNT(*) as cantidad,
                SUM(tp.amount) as monto_original,
                COALESCE(SUM(
                    CASE WHEN tp.currency_code = 'VES'
                         THEN tp.amount / NULLIF(tp.exchange_rate_used, 0)
                         ELSE tp.amount
                    END
                ), 0) as total_usd,
                COALESCE(SUM(tp.amount_in_bs), SUM(
                    CASE WHEN tp.currency_code = 'VES'
                         THEN tp.amount
                         ELSE tp.amount * {$bcv_rate}
                    END
                )) as total_bs
            ")
            ->groupBy('tp.method', 'tp.currency_code')
            ->orderBy('tp.method')
            ->get();

        // Devoluciones del día
        $devoluciones_usd = DB::table('transactions')
            ->where('business_id', $business_id)
        ...
        // Gastos del día
        $gastos_usd = DB::table('transactions')
            ->where('business_id', $business_id)
            ->where('type', 'expense')
            ->whereDate('transaction_date', $date)
            ->when($location_id, fn($q) => $q->where('location_id', $location_id))
        ...
        $resumen = [
            ...
            'total_ventas_bs'       => (float)$total_ventas_usd * $bcv_rate,
            'total_transacciones'   => (int)$total_transacciones,
            'devoluciones_usd'      => (float)$devoluciones_usd,
            'gastos_usd'            => (float)$gastos_usd,
            'neto_usd'              => (float)$total_ventas_usd - (float)$devoluciones_usd - (float)$gastos_usd,
            'fondo_inicial'         => $cash_register ? (float)$cash_register->opening_amount : 0,
        ];
        $resumen['neto_bs'] = $resumen['neto_usd'] * $bcv_rate;

        $locations = BusinessLocation::forDropdown($business_id, true);

        return view('report.cash_register_close', compact(
    ...
    * Actualiza datos operativos desde la tabla dinámica del reporte de cierre.
    ...
    // =========================================================================
    // EXPORTACIONES CSV — Ventas Diarias, Ventas Mensuales, Cierre de Caja
    // =========================================================================
    ...
    public function exportDailySales(Request $request)
    {
        ...
        $transactions = DB::table('transactions as t')
            ...
            ->whereIn('t.status', ['final'])
            ->when($location_id, fn($q) => $q->where('t.location_id', $location_id))
            ->selectRaw('t.invoice_no, t.transaction_date, c.name as cliente, t.final_total,
                         t.final_total * ? as total_bs, t.payment_status', [$bcv_rate])
            ->orderBy('t.transaction_date')
            ->get();

        $filename = 'ventas_diarias_' . $date . '.csv';
        $headers  = [
            'Content-Type'        => 'text/csv; charset=UTF-8',
            'Content-Disposition' => 'attachment; filename="' . $filename . '"',
        ];

        $callback = function () use ($transactions, $date, $bcv_rate) {
            $out = fopen('php://output', 'w');
            fprintf($out, chr(0xEF) . chr(0xBB) . chr(0xBF)); // BOM UTF-8
            fputcsv($out, ['Reporte de Ventas Diarias — ' . $date . ' — Tasa BCV: ' . number_format($bcv_rate, 2)]);
            fputcsv($out, []);
            fputcsv($out, ['N° Factura', 'Fecha/Hora', 'Cliente', 'Total USD', 'Total Bs', 'Estado Pago']);
            foreach ($transactions as $row) {
                fputcsv($out, [
                    ...
                    number_format($row->total_bs, 2, '.', ''),
                    $row->payment_status,
                ]);
            }
            fputcsv($out, []);
            fputcsv($out, ['TOTAL', '', '', number_format($transactions->sum('final_total'), 2, '.', ''),
                number_format($transactions->sum('total_bs'), 2, '.', ''), '']);
            fclose($out);
        };
    ...
    /**
     * Exporta Reporte de Ventas Mensuales a CSV.
     * GET /reports/monthly-sales/export
     */
    public function exportMonthlySales(Request $request)
    {
        ...
        $filename  = 'ventas_mensuales_' . $year . '_' . str_pad($month, 2, '0', STR_PAD_LEFT) . '.csv';
        $headers   = [
            'Content-Type'        => 'text/csv; charset=UTF-8',
            'Content-Disposition' => 'attachment; filename="' . $filename . '"',
        ];

        $callback = function () use ($daily_data, $monthName, $bcv_rate) {
            $out = fopen('php://output', 'w');
            fprintf($out, chr(0xEF) . chr(0xBB) . chr(0xBF));
            fputcsv($out, ['Reporte de Ventas Mensuales — ' . $monthName . ' — Tasa BCV: ' . number_format($bcv_rate, 2)]);
            fputcsv($out, []);
            fputcsv($out, ['Fecha', 'N° Transacciones', 'Total USD', 'Total Bs']);
            ...
            fputcsv($out, []);
            fputcsv($out, ['TOTAL', $daily_data->sum('transactions'),
                number_format($daily_data->sum('total_usd'), 2, '.', ''),
                number_format($daily_data->sum('total_bs'), 2, '.', '')]);
            fclose($out);
        };
    ...
    /**
     * Exporta Reporte de Cierre de Caja a CSV.
     * GET /reports/cash-register-close/export
     */
    public function exportCashRegisterClose(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'No autorizado');
        }
        ...
        $cobros = DB::table('transaction_payments as tp')
            ->join('transactions as t', 't.id', '=', 'tp.transaction_id')
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->whereIn('t.status', ['final'])
            ...
            ->selectRaw("
                ...
                SUM(tp.amount) as monto_original,
                COALESCE(SUM(CASE WHEN tp.currency_code='VES' THEN tp.amount/NULLIF(tp.exchange_rate_used,0) ELSE tp.amount END),0) as total_usd,
                COALESCE(SUM(tp.amount_in_bs), SUM(CASE WHEN tp.currency_code='VES' THEN tp.amount ELSE tp.amount * {$bcv_rate} END)) as total_bs
            ")
            ->groupBy('tp.method', 'tp.currency_code')
            ->orderBy('tp.method')
            ->get();

        $total_ventas_usd = DB::table('transactions')
            ->where('business_id', $business_id)->where('type', 'sell')
            ->whereIn('status', ['final'])->whereDate('transaction_date', $date)
            ->when($location_id, fn($q) => $q->where('location_id', $location_id))
            ->sum('final_total');

        $filename = 'cierre_caja_' . $date . '.csv';
        $headers  = [
            'Content-Type'        => 'text/csv; charset=UTF-8',
            'Content-Disposition' => 'attachment; filename="' . $filename . '"',
        ];

        $callback = function () use ($cobros, $total_ventas_usd, $bcv_rate, $date) {
            $out = fopen('php://output', 'w');
            fprintf($out, chr(0xEF) . chr(0xBB) . chr(0xBF));
            fputcsv($out, ['Cierre de Caja — ' . $date . ' — Tasa BCV: ' . number_format($bcv_rate, 2)]);
            fputcsv($out, []);
            fputcsv($out, ['TOTAL VENTAS USD', number_format($total_ventas_usd, 2, '.', '')]);
            fputcsv($out, ['TOTAL VENTAS Bs', number_format($total_ventas_usd * $bcv_rate, 2, '.', '')]);
            fputcsv($out, []);
            fputcsv($out, ['Método de Pago', 'Moneda', 'Cantidad', 'Monto Original', 'Total USD', 'Total Bs']);
            foreach ($cobros as $row) {
                fputcsv($out, [
                    $row->metodo,
                    $row->moneda,
                    $row->cantidad,
                    number_format($row->monto_original, 2, '.', ''),
                    number_format($row->total_usd, 2, '.', ''),
                    number_format($row->total_bs, 2, '.', ''),
                ]);
            }
            fputcsv($out, []);
            fputcsv($out, ['TOTAL COBROS', '', $cobros->sum('cantidad'), '',
                number_format($cobros->sum('total_usd'), 2, '.', ''),
                number_format($cobros->sum('total_bs'), 2, '.', '')]);
            fclose($out);
        };
