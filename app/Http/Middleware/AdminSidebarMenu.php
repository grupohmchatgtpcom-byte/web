        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            <path d="M10 12h4v4h-4z" />
                        ...
                        $menu->dropdown(
                                ...
                                function ($sub) {
                                        $sub->url(
                                                action([\App\Http\Controllers\HomeController::class, 'showBcvRate']),
                                                'Ver tasa del dia',
                                                ['icon' => '', 'active' => request()->segment(1) == 'bcv-rate']
                                        );
                                },
            ...
            if (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create')) {
                $menu->url(
                    action([\App\Http\Controllers\ConsolidadoController::class, 'index']),
                    'Consolidado',
                    [
                        'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5 tw-shrink-0" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                        <path d="M5 4h14a1 1 0 0 1 1 1v5a1 1 0 0 1 -1 1h-7a1 1 0 0 0 -1 1v3a1 1 0 0 1 -1 1h-6a1 1 0 0 1 -1 -1v-10a1 1 0 0 1 1 -1z" />
                        <path d="M12 8h4" />
                        <path d="M8 8h1" />
                        <path d="M8 12h1" />
                        <path d="M5 16h1" />
                        ...
                        'active' => request()->segment(1) == 'consolidado',
                    ]
                )->order(7);
            }
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        // ===== Reportes GHM Venezuela =====
                        if ($is_admin || auth()->user()->can('purchase_n_sell_report.view')) {
                            $sub->url(
                                route('reports.daily_sales'),
                                'Facturación Diaria Bs/$',
                                ['icon' => '', 'active' => request()->segment(2) == 'daily-sales']
                            );
                            $sub->url(
                                route('reports.monthly_sales'),
                                'Facturación Mensual Bs/$',
                                ['icon' => '', 'active' => request()->segment(2) == 'monthly-sales']
                            );
                            $sub->url(
                                route('reports.cxc'),
                                'Cuentas por Cobrar (CxC)',
                                ['icon' => '', 'active' => request()->segment(2) == 'accounts-receivable']
                            );
                            $sub->url(
                                route('reports.purchases_period'),
                                'Compras del Período',
                                ['icon' => '', 'active' => request()->segment(2) == 'purchases-period']
                            );
                        }
                        if ($is_admin || auth()->user()->can('tax_report.view')) {
                            $sub->url(
                                route('reports.libro_ventas'),
                                'Libro de Ventas (SENIAT)',
                                ['icon' => '', 'active' => request()->segment(2) == 'libro-ventas']
                            );
                            $sub->url(
                                route('reports.libro_compras'),
                                'Libro de Compras (SENIAT)',
                                ['icon' => '', 'active' => request()->segment(2) == 'libro-compras']
                            );
                        }
                        if ($is_admin || auth()->user()->can('register_report.view')) {
                            $sub->url(
                                route('reports.cash_register_close'),
                                'Cierre de Caja',
