...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'sync_status' => ! empty($input['sync_status']) ? $input['sync_status'] : 'synced',
            'origin_device_id' => ! empty($input['origin_device_id']) ? $input['origin_device_id'] : null,
            'origin_location_id' => ! empty($input['origin_location_id']) ? $input['origin_location_id'] : null,
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'sync_status' => !empty($payment['sync_status']) ? $payment['sync_status'] : 'synced',
                    ...
                    // Dualidad de monedas: registrar moneda, tasa y equivalencias al momento del pago
                    $currencyCode = ! empty($payment['currency_code']) ? strtoupper(trim($payment['currency_code'])) : 'USD';
                    $rateUsed = ! empty($payment['exchange_rate_used'])
                        ? (float) $payment['exchange_rate_used']
                        : getActiveBcvRate();
                    if ($rateUsed <= 0) {
                        $rateUsed = 1.0;
                    }
                    if ($currencyCode === 'VES' || $currencyCode === 'BS' || $currencyCode === 'VEF') {
                        $amountInUsd = $rateUsed > 0 ? round($payment_amount / $rateUsed, 4) : $payment_amount;
                        $amountInBs  = round($payment_amount, 4);
                    } else {
                        // USD o cualquier otra moneda base
                        $amountInUsd = round($payment_amount, 4);
                        $amountInBs  = round($payment_amount * $rateUsed, 4);
                    }
                    $payment_data['currency_code']       = $currencyCode;
                    $payment_data['exchange_rate_used']  = $rateUsed;
                    $payment_data['amount_in_usd']       = $amountInUsd;
                    $payment_data['amount_in_bs']        = $amountInBs;
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        // Dualidad de monedas: recalcular equivalencias al editar el pago
        $currencyCode = ! empty($payment['currency_code']) ? strtoupper(trim($payment['currency_code'])) : 'USD';
        $rateUsed     = ! empty($payment['exchange_rate_used']) && (float) $payment['exchange_rate_used'] > 0
            ? (float) $payment['exchange_rate_used']
            : getActiveBcvRate();
        if ($rateUsed <= 0) {
            $rateUsed = 1.0;
        ...
        if ($currencyCode === 'VES' || $currencyCode === 'BS' || $currencyCode === 'VEF') {
            $payment['amount_in_usd'] = $rateUsed > 0 ? round($payment['amount'] / $rateUsed, 4) : $payment['amount'];
            $payment['amount_in_bs']  = round($payment['amount'], 4);
        } else {
            $payment['amount_in_usd'] = round($payment['amount'], 4);
            $payment['amount_in_bs']  = round($payment['amount'] * $rateUsed, 4);
        }
        $payment['currency_code']      = $currencyCode;
        $payment['exchange_rate_used'] = $rateUsed;
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
                ...
                if (! empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        $bs_extra = [
                            ...
                            'amount_in_bs'      => $value['amount_in_bs'] ?? null,
                            'exchange_rate_used' => $value['exchange_rate_used'] ?? null,
                        ];
                        if ($value['method'] == 'cash') {
                            $output['payments'][] = array_merge(
        ...
        // GHM: Tasa BCV y total en Bolívares para mostrar en el recibo
        $ghm_bcv = !empty($transaction->ghm_bcv_rate) && $transaction->ghm_bcv_rate > 1
            ? (float) $transaction->ghm_bcv_rate
            : (float) getActiveBcvRate();
        if ($ghm_bcv > 1) {
            $output['bcv_rate_value']    = $ghm_bcv;
            $output['bcv_rate_date']     = \Carbon\Carbon::parse($transaction->transaction_date)->format('d/m/Y');
            $output['total_bs_display']  = number_format($transaction->final_total * $ghm_bcv, 2, ',', '.');
        }
